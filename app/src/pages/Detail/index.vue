        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转：从A路由跳转到B路由，这里加入购物车，进行路由跳转之前，发请求把购买的
                产品信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      // 派发action
      // 1.发请求---将产品加入到数据库
      /* 当前这里派发action,向服务器发请求,判断加入购物车是成功了还是失败,进行相应的操作.
          调用仓库中的addOrderUpdateShopCart,这个方法加上async,返回一定是一个promise
          要么成功,要么失败 */
      try {
        ...
        // 路由跳转
        /* 在路由跳转的时候需要将产品的信息带给下一级的路由组件
          一些简单的数据skuNum,通过query参数形式给路由组件传递过去
          产品的信息[计较复杂:skuInfo],通过会话存储(不持久化,会话结束数据就消失)
          本地存储|会话存储,一般存储的是字符串 */
