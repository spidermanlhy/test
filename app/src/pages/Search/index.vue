...
export default {
  ...
  watch: {
    // 监听路由的信息是否发生变化，如果发生变化，再次发出请求
    $route(newValue, oldValue) {
      // 每一次请求完毕，应该把相应的1、2、3级分类的id置空，让他接收下一次的1、2、3id
  ...
  methods: {
    ...
    attrInfo(attr, attrValue) {
      ...
      // 向searchParams.props数组push新元素props(只有数组里没有此数据才往数组里push)
    ...
    // 排序的操作
    changeOrder(flag){
      // flag形参：它是一个标记，代表用户点击的是综合（1）价格（2）【用户点击的时候传入】
      // 传入searchParams里的order
      let originOrder = this.searchParams.order;
      // 这里获取最开始的状态
      let originFlag = this.searchParams.order.split(":")[0];
      let originSort = this.searchParams.order.split(":")[1];
      // 准备一个新的order属性以便赋值给searchParams
      let newOrder = "";
      // 点击的是综合
      if(flag==originFlag){
